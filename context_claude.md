...
9.  **📋 Log Everything**: Ensure session log (e.g., `R/setup/fix_issue_123.R`) was included in the PR.
...
We use a 3-file strategy (`package.nix`, `default.nix`, `packages.R`) managed by `R/setup/maintain_env.R`.
...
**File Structure:**
- `R/`: Package code.
- `R/setup/`: Housekeeping, logs, bug fix scripts.
- `R/tar_plans/`: Targets plans.
- `inst/qmd/`: Source Quarto files (README, vignettes).

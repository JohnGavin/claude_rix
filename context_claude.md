...
**See Wiki**: [`Environment-Sync-and-Nix-Generation.md`](../../WIKI_CONTENT/Environment-Sync-and-Nix-Generation.md) for detailed implementation.
...
2. **`default-ci.nix`** (or `default.nix` directly): CI/dev environment
   - Built from DESCRIPTION all deps + dev tools
   - Generated by: `maintain_env.R` calling `rix::rix()`
   - Used by: Local development, GitHub Actions for testing/building

3. **`packages.R`**: Helper file listing packages for `default.R`

#### Using `R/setup/maintain_env.R`

We have standardized on a robust maintenance script `R/setup/maintain_env.R` that:
1.  Reads `DESCRIPTION`.
2.  Generates `packages.R`.
3.  Generates `default.nix` with a **fixed shell hook** (resolving the `$HOME` artifact bug).
4.  Generates `package.nix` for Cachix.

**Workflow:**
```r
# To update environment after DESCRIPTION changes:
source("R/setup/maintain_env.R")
maintain_env()
```

The `default.R` file now simply sources this script:
```r
source("R/setup/maintain_env.R")
```

This is integrated into the `targets` pipeline to ensure automatic updates.
